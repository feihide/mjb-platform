...
function extractCredentials(done) {
  ...
  // Grant type
  // console.log(this.req.body.grant_type);
  ...
  } else if (!this.client.clientSecret && this.grantType != 'refresh_token') {
...
function checkClient(done) {
  ...
  this.model.getClient(this.client.clientId, this.client.clientSecret,
    function(err, client) {
      ...
      if (!client) {
        return done(error('invalid_client', 'Client credentials are invalid in checkClient'));
      }
...
function checkTarget(done) {
  this.targetId = this.req.body && this.req.body.targetId;
  if (this.targetId) {
    var self = this;
    this.model.getClient(this.targetId, null,
      function(err, client) {

        if (err) return done(error('server_error', false, err));
        if (!client) {
          return done(error('invalid_client', 'Client invalid in checkTarget'));
        }

        // Expose validated client
        self.req.body.reqeustUri = client
        done();
      });
  } else {
    done();
...
function generateRefreshToken(done) {
  if (this.config.grants.indexOf('refresh_token') === -1 || this.grantType == 'refresh_token') return done();
