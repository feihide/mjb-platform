  ...
  return {
    findAll: function*(next) {
      ...
      try {
        ...
        //判断是否存在需要模糊查询的字段
        for (var i in query) {
          if (query[i] && i.indexOf('search_') != -1) {
            conditions[i.substr(7)] = new RegExp(query[i], "i");
          }

          if (query[i] && i.indexOf('begin_') != -1) {
            conditions[i.substr(6)] = {
              $gte: query[i]
            };
          }
          if (query[i] && i.indexOf('end_') != -1) {
            if (!conditions[i.substr(4)]) {
              conditions[i.substr(4)] = {}
            }
            conditions[i.substr(4)].$lte = query[i]
          }
        }

        //支持单库添加
        if (query.city) {
          if (query.city == 'common') {
            conditions.city = {
              $in: this.request.openCity
            }
          ...
          if (model.schema.path('is_del') != undefined) {
            if (conditions.is_del != 1) {
              conditions.is_del = 0
            }
          }
        }
    ...
    updateById: function*(next) {
      ...
      var error,
        result;
      try {
        this.request.body.utime = parseInt(new Date().getTime() / 1000)
    ...
    create: function*(next) {
      ...
      var error,
        result;
      try {
        this.request.body.ctime = this.request.body.utime = parseInt(new Date().getTime() / 1000)
